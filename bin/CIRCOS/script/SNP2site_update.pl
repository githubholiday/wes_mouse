...
my $inputfile_SNP = shift;#'test.vcf'; #shift;#'test.vcf';
my $outputdir =shift;#'b';# shift; #'b';
my $need_chrome = shift;#'auto'; #shift ;#'auto';
my $need_window = shift;#1000000 ;#shift; #1000000;
my $db_reference_fai = shift;#'ucsc.hg19.fasta.fai';
my $need_tag =shift;
		...
		foreach my $snp (@{$hash_SNP_VCFinfos{$key}{$key_chr}}) {
			if ($key eq 'Single') {
				...
				if ($key eq 'Group') {
					...
					#print "$str_chr\t($hash_SNP_F{$key_chr}{$key}{'MAX'}-$hash_SNP_F{$key_chr}{$key}{'MIN'})\n";
					my $temp=$hash_SNP_F{$key_chr}{$key}{'MAX'}-$hash_SNP_F{$key_chr}{$key}{'MIN'};
					if($temp eq 0){
					}else{
