...
def main():
	...
	input = '{0}/{1}.{2}.{3}'.format(args.indir,args.sample,args.type,'hg19_multianno.txt')
	...
	#切割染色体
	OUTFILE = open('{0}/{1}.{2}.{3}.{4}'.format(outdir,args.sample,firstchr,args.type,'hg19_multianno.txt'),'w')
	...
	with open (input,'r') as infile:
		for line in infile:
			...
			if tmp[0] == firstchr:
				...
				OUTFILE.close()
				if tmp[0] not in log:
					OUTFILE = open('{0}/{1}.{2}.{3}.{4}'.format(outdir,args.sample,tmp[0],args.type,'hg19_multianno.txt'),'w')
					OUTFILE.write(header)
				else:
					OUTFILE = open('{0}/{1}.{2}.{3}.{4}'.format(outdir,args.sample,tmp[0],args.type,'hg19_multianno.txt'),'a')
				OUTFILE.write( line )
				firstchr = tmp[0]
				log[firstchr] = 1
	OUTFILE.close()
