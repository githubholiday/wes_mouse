...
def link_fq(filter, outdir, raw_sample, new_sample):
	#os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}.base.png {2}/{3}/{3}.base.png".format(filter, sample, outdir))
	#os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}.base.pdf {2}/{3}/{3}.base.pdf".format(filter, sample, outdir))
	#os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}.quality.pdf {2}/{3}/{3}.quality.pdf".format(filter, sample, outdir))
	#os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}.quality.png {2}/{3}/{3}.quality.png".format(filter, sample, outdir))
	#os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}.quality.png {2}/{3}/{3}.quality.png".format(filter, sample, outdir))
	os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}_R1.fq.gz {2}/{3}/{3}_R1.fq.gz".format(filter, raw_sample, outdir, new_sample))
	os.system("ln -s {0}/Analysis/{1}/filter/clean/{1}_R2.fq.gz {2}/{3}/{3}_R2.fq.gz".format(filter, raw_sample, outdir, new_sample))
			...
			for filter in filter_all:
				if not os.path.exists('{0}/Analysis/{1}'.format(filter,bms)):
					...
					if bms == info:
						...
						link_fq(filter, outdir, bms, info)
